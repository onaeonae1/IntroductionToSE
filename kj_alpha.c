		...
		if (MD.category_alpha == 1) {
			...
			case 2: // 1.2 timekeeping_change_sec
				...
				case 1: // A
					...
					break;
				case 2: // B
					if (CT.SS == 59) CT.SS = 0; // 최대치가 된 상태에서 다시 입력하면 최저값으로
			...
			case 3: // 1.3 timekeeping_change_hr
				...
				case 1: // A
					...
					break;
				case 2: // B
					if (CT.HH == 23) CT.HH = 0; // 최대치가 된 상태에서 다시 입력하면 최저값으로
			...
			case 4: // 1.4 timekeeping_change_min
				...
				case 1: // A
					...
					break;
				case 2: // B
					if (CT.MM == 59) CT.MM = 0; // 최대치가 된 상태에서 다시 입력하면 최저값으로
			...
			case 5: // 1.5 timekeeping_change_yr
				...
				case 1: // A
					...
					break;
				case 2: // B
					if (CT.YY == 99) CT.YY = 19; // 최대치가 된 상태에서 다시 입력하면 최저값으로
			...
			case 6: // 1.6 timekeeping_change_mnth
				...
				case 1: // A
					...
					break;
				case 2: // B
					if (CT.MT == 12) CT.MT = 1; // 최대치가 된 상태에서 다시 입력하면 최저값으로
			...
			case 7: // 1.7 timekeeping_change_day
				...
				case 2: // B
					...
					case 10:
					case 12:
						if (CT.DD == 31) CT.DD = 1; // 최대치가 된 상태에서 다시 입력하면 최저값으로
						else CT.DD++;
					...
					case 2:
						if(CT.YY % 4 == 0) { // 윤년이면
							if(CT.DD == 29) CT.DD = 1; // 최대치가 된 상태에서 다시 입력하면 최저값으로
							else CT.DD++;
						}
						else { // 윤년이 아닌 경우에는
							if(CT.DD == 29) CT.DD = 1;
					...
					case 9:
					case 11:
						if (CT.DD == 30) CT.DD = 1; // 최대치가 된 상태에서 다시 입력하면 최저값으로
						else CT.DD++;
						// stopwatch 시작 일도 같이 늘려주기
...
void Realtime_Manager() {
	...
	case 11:
		...
		break;
	case 2:
		if(ST.stopwatchTime.YY % 4 == 0 && ST.stopwatchTime.DD > 29) { // 윤년이고 일 수>29인 경우
			ST.stopwatchTime.DD -= 29;
			ST.stopwatchTime.MT++;
		}
		else if(ST.stopwatchTime.YY % 4 != 0 && ST.stopwatchTime.DD > 28) { // 윤년이 아니고 일 수 > 28인 경우
	...
	case 2:
		...
		else if(ST.lapTime.YY % 4 == 0 && ST.lapTime.DD > 29) {
			ST.lapTime.DD -= 28;
			ST.lapTime.MT++;
		}
		break;
